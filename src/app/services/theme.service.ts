...
export class ThemeService {
  ...
  )
  {
   for (var theme in Theme.Mode) {
     if (this.document.documentElement.classList.contains(Theme.getTheme(theme))) {
   ...
   Theme.currentTheme =
    ...
    if(localStorage[LocalConfig.theme]) this.selectTheme(localStorage[LocalConfig.theme] as Theme.Mode);

   this.selectTheme(
     Theme.getTheme(Theme.currentTheme)
   );
  ...
  selectTheme(theme: Theme.Mode): void {
    ...
    for (var themeKey in Theme.Mode) {
      this.document.documentElement.classList.remove(Theme.Mode[themeKey as keyof typeof Theme.Mode]);
    }
  ...
  toggleTheme(): void {
    this.selectTheme(Theme.currentTheme as Theme.Mode == Theme.Mode.light ? Theme.Mode.dark : Theme.Mode.light)
  }
